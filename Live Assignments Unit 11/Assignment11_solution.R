...
# Number 1: Brief Financial Data
tsData<-EuStockMarkets[,"DAX"]
plot(tsData, col="blue", main="European Stock Market Data, 1990 onward", ylab="Stock Index", xlab="Year")
abline(v=1997, col="red")

decompRes<-decompose(tsData, type="mult")
plot(decompRes, col="blue")
abline(v=1997, col="red")

# Number 2: In this exercise, we will use Temperature data
...
# Number 3
df<-read.csv("/Users/faizan/Downloads/Unit12TimeSeries_Ollivander.csv", header=FALSE)
df$V1<-as.Date(df$V1, format="%m/%d/%Y")
df<-xts(df$V2, order.by=df$V1)

df2<-read.csv("/Users/faizan/Downloads/Unit12TimeSeries_Gregorovitch.csv", header=FALSE)
